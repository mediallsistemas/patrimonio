...
export default function DraftBanner({ draftServidor, retomar, descartarDraft }: Props) {
  const feitos = draftServidor.concluidos?.length ?? 0
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs font-sans text-amber-700 mt-0.5">
            Você tem uma ronda pausada com {feitos} local{feitos !== 1 ? 'is' : ''} registrado{feitos !== 1 ? 's' : ''}.
            {draftServidor.iniciadoEm && ` Iniciada às ${new Date(draftServidor.iniciadoEm).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}.`}
          </p>
        </div>
      </div>
