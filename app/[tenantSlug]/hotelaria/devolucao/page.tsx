...
export default function DevolucaoPage() {
  ...
  async function confirmarDevolucao() {
    ...
    setConfirmando(true)
    try {
      await api.post(`hotelaria/${tenantSlug}/movimentacoes`, { pessoaId: pessoa.id, tipo: 'devolucao' })
      setEstado('sucesso')
    } catch (err: unknown) {
          ...
          {estado === 'com_pendentes' && pessoa && (
            <Card shadow="md" padding="lg">
              <div className="text-center space-y-5">
                ...
                <Text variant="body-md" className="text-gray-300 block">Deseja confirmar a devolução de 1 item?</Text>
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setEstado('camera')} className="flex-1">Cancelar</Button>
                  <Button variant="success" onClick={confirmarDevolucao} disabled={confirmando} className="flex-1">
          ...
          {estado === 'sem_pendentes' && pessoa && (
            <Card shadow="md" padding="lg">
              <div className="text-center space-y-5">
                ...
                <Text as="h2" variant="heading-sm" className="text-dark block">Olá, {pessoa.nome}!</Text>
                <div className="bg-yellow-light rounded-xl p-4 border border-gray-200">
                  <Text as="p" variant="body-md" className="text-yellow-dark">Você não possui retiradas pendentes de devolução.</Text>
                </div>
                <Button onClick={() => router.push(`/${tenantSlug}/hotelaria`)} className="w-full">Voltar ao Menu</Button>
              </div>
            </Card>
          )}
          ...
          {estado === 'nao_encontrado' && (
            <Card shadow="md" padding="lg">
              <div className="text-center space-y-5">
                ...
                <Text variant="body-md" className="text-gray-300 block">Não foi possível identificar nenhum usuário cadastrado.</Text>
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setEstado('camera')} className="flex-1">Tentar novamente</Button>
                  <Button variant="secondary" onClick={() => router.push(`/${tenantSlug}/hotelaria`)} className="flex-1">Voltar</Button>
                </div>
              </div>
            </Card>
          )}
          ...
          {estado === 'sucesso' && (
            <Card shadow="md" padding="lg">
              <div className="text-center space-y-5">
                ...
                <Text as="h2" variant="heading-md" className="text-dark block">Devolução registrada!</Text>
                <Text variant="body-md" className="text-gray-300 block">Obrigado, {pessoa?.nome}! A devolução foi confirmada com sucesso.</Text>
                <Button onClick={() => router.push(`/${tenantSlug}/hotelaria`)} className="w-full" size="lg">Voltar ao Menu</Button>
              </div>
            </Card>
          )}
