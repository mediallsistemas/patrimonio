...
import type { DetalheAlteracao } from '../inspecao.types'
import { TIPOS_ALTERACAO } from '../inspecao.types'

interface Props {
  ambiente: string
  detalhe: DetalheAlteracao
...
export default function AlteracaoDetalhe({ ambiente, detalhe, errors, onChange, onClearError, fileInputRef, onFoto, onVoltar, onNext }: Props) {
  return (
    <Card shadow="md">
      ...
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {TIPOS_ALTERACAO.map(({ value, label, color }) => (
              <button key={value} type="button"
                onClick={() => { onChange({ ...detalhe, tipo: value }); onClearError('tipo') }}
        ...
        <div className="space-y-1.5">
          ...
          <textarea rows={4}
            ...
            onChange={(e) => { onChange({ ...detalhe, descricao: e.target.value }); onClearError('descricao') }}
        ...
        <div className="space-y-2">
          ...
          {detalhe.foto ? (
            <div className="relative rounded-xl overflow-hidden border border-gray-200">
              ...
              <button type="button"
                onClick={() => onChange({ ...detalhe, foto: null })}
