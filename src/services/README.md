...
| `manutencoes.service.ts` | `me/manutencoes`, `me/manutencoes/{id}/finalizar`, `me/bens/buscar`, `manutencoes/realizadas`, `manutencoes/historico` | `iniciar(input)` (union `IniciarAmbienteInput` eletrica/hidraulica × `IniciarPatrimonioInput`), `finalizar(id, { fotoDepois, observacaoFinal? })`, `buscarBemPorPatrimonio(patrimony)`, `listarRealizadasPorAssets(assetIds)`, `buscarRealizadaDetalhe(id)`, `listarHistorico()` |
