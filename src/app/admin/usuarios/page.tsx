...
type ResetState =
  | { status: 'idle' }
  | { status: 'confirming'; id: string; nome: string }
  | { status: 'loading' }
  | { status: 'done'; novaSenha: string; nome: string }
...
export default function UsuariosPage() {
  ...
  async function handleReset(id: string) {
    ...
    try {
      ...
      const json = await res.json() as { data?: { novaSenha: string }; novaSenha?: string }
      const novaSenha = json.data?.novaSenha ?? json.novaSenha ?? ''
      const nome = usuarios.find((u) => u.id === id)?.nome ?? ''
      setResetState({ status: 'done', novaSenha, nome })
    } catch {
  ...
  return (
    <div className="form-bg min-h-screen flex flex-col items-center p-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {isSuperAdmin && (
                    <button
                      ...
                      className="shrink-0 p-2 rounded-lg text-gray-300 hover:text-orange-600 hover:bg-orange-50 transition-colors"
                    >
      ...
      {resetState.status === 'confirming' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <p className="text-sm font-sans text-dark font-semibold mb-1">Resetar senha</p>
            <p className="text-sm font-sans text-gray-300 mb-5">
              Gerar nova senha temporária para <span className="text-dark font-semibold">{resetState.nome}</span>?
            </p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setResetState({ status: 'idle' })}
                className="text-sm font-sans px-4 py-2 rounded-lg border border-gray-200 text-gray-300 hover:text-dark transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={() => { void handleReset(resetState.id) }}
                className="text-sm font-sans px-4 py-2 rounded-lg text-white transition-colors"
      ...
      {resetState.status === 'loading' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-6">
            <p className="text-sm font-sans text-gray-300">Gerando nova senha...</p>
          </div>
        </div>
      )}
      ...
      {resetState.status === 'done' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <p className="text-sm font-sans text-dark font-semibold mb-1">Nova senha gerada</p>
            <p className="text-sm font-sans text-gray-300 mb-3">
              Senha temporária para <span className="text-dark font-semibold">{resetState.nome}</span>:
            </p>
            <div className="bg-gray-50 rounded-lg px-4 py-3 font-mono text-base text-dark tracking-widest text-center mb-5 select-all">
              {resetState.novaSenha}
            </div>
            <p className="text-xs text-gray-300 font-sans mb-5">O usuário será obrigado a trocar a senha no próximo login.</p>
            <button
              onClick={() => setResetState({ status: 'idle' })}
              className="w-full text-sm font-sans px-4 py-2 rounded-lg bg-blue-dark text-white hover:opacity-90 transition-opacity"
            >
              Fechar
            </button>
          </div>
