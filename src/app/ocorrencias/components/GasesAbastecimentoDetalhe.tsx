...
import { TAMANHOS_CILINDRO, type DraftEstado } from '@/app/ocorrencias/types'

interface GasesAbastecimentoDetalheProps {
  estado: DraftEstado
    ...
    <Card shadow="md">
      ...
      <div className="space-y-5">
        <Input
          ...
          onChange={(e) =>
            atualizar({ abastecimento: { ...estado.abastecimento, quantidade: e.target.value } })
          }
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            {TAMANHOS_CILINDRO.map((tam) => (
              <button
                ...
                type="button"
                onClick={() =>
                  atualizar({ abastecimento: { ...estado.abastecimento, tamanho: tam } })
                }
          ...
          {errors.tamanho && (
            <span className="text-xs text-red-base font-sans">{errors.tamanho}</span>
          )}
        </div>
      </div>
      <div className="flex gap-3 mt-6">
        <Button
          variant="outline"
          onClick={() => atualizar({ etapa: 'gases_abastecimento' })}
          className="flex-1"
        >
          Voltar
        </Button>
