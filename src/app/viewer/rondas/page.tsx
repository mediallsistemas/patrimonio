...
import {
  ...
  AlertCircle, Eye, Building2, ArrowLeft,
} from 'lucide-react'
...
function RondaListItem({ ronda, selecionada, onClick }: {
  ronda: Ronda
  selecionada: boolean
  onClick: () => void
}) {
  ...
  const horas = emAndamento ? differenceInHours(new Date(), new Date(ronda.iniciadoEm)) : 0
  ...
  const duracao = !emAndamento && ronda.finalizadoEm
    ? Math.round((new Date(ronda.finalizadoEm).getTime() - new Date(ronda.iniciadoEm).getTime()) / 60000)
    : null
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-gray-400 tabular-nums flex-wrap">
            ...
            {ronda.ambientes.length > 0 && (<><span className="text-gray-200">·</span><span>{ronda.ambientes.length} amb.</span></>)}
            {ronda.criadoPor && (<><span className="text-gray-200">·</span><span>{ronda.criadoPor.nome}</span></>)}
          </div>
        </div>
...
export default function ViewerRondasPage() {
  const { user } = useAuth()
  ...
  const { data: rondas = [], isLoading } = useQuery({
    queryKey: ['viewer-rondas'],
    queryFn: listarRondas,
    refetchInterval: 30_000,
  })

  const { data: tenants = [] } = useQuery<Tenant[]>({
    queryKey: ['viewer-tenants'],
  ...
  const rondasComOcorrencia = rondasFiltradas.filter((r) => r.ambientes.some((a) => a.temOcorrencia))
    ...
    <div className="min-h-screen bg-[#f8f9fb] font-sans">
      ...
      <header className="bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/viewer" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 transition-colors">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 text-xs font-medium text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-full ring-1 ring-indigo-100">
              <Eye className="w-3.5 h-3.5" />
              Visualizador · {user?.nome}
            </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label="Total de rondas" value={totalRondas} icon={Activity} iconBg="bg-indigo-50" iconColor="text-indigo-500" valueColor="text-gray-900" />
          <KpiCard
        ...
        {isLoading ? (
          ...
          <div className="space-y-10">
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-2">
                  {rondasOrdenadas
                    ...
                    .map((r) => (
                      <RondaListItem key={r.id} ronda={r} selecionada={r.id === rondaSelecionadaId} onClick={() => selecionarRonda(r.id)} />
                    ))}
                </div>
              </section>
            )}

            {rondasOrdenadas.length === 0 && !isLoading && (
              <div className="flex flex-col items-center py-16 gap-3 text-center bg-white rounded-xl ring-1 ring-gray-100">
                ...
                <p className="text-sm font-semibold text-gray-700">Nenhuma ronda encontrada</p>
                <p className="text-xs text-gray-400">Nenhuma ronda registrada para as unidades vinculadas.</p>
              </div>
            )}
                ...
                {rondaSelecionada && (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${verConformes ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
                        {conformes.length}
                      </span>
                    </button>
                  )
              ...
              {verConformes && rondaSelecionada ? (() => {
                ...
                return (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {conformes.map((a) => (
                      <div key={a.id} className="bg-white rounded-xl ring-1 ring-gray-100 px-4 py-3 flex items-center gap-3">
                ...
                <div className="space-y-2.5">
                  {ocorrencias.map((oc) => (
                    <OcorrenciaRow key={`${oc.rondaId}-${oc.ambienteId}-${oc.id}`} oc={oc} />
                  ))}
                </div>
              ) : (
                <div className="flex flex-col items-center py-12 gap-3 text-center bg-white rounded-xl ring-1 ring-gray-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {rondaSelecionada ? 'Esta ronda foi concluída em total conformidade.' : 'Todas as rondas foram concluídas em conformidade.'}
                    </p>
                  </div>
                </div>
              )}
            ...
            {/* Conformidade colapsável */}
            {!soOcorrencias && rondasNormais.length > 0 && (
              <section className="space-y-3">
                <button onClick={() => setNormalAberto((v) => !v)} className="flex items-center gap-2 group">
                  <SectionHeader
                ...
                {normalAberto && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                    {rondasNormais.map((r) => (
                      <div key={r.id} className="bg-white rounded-xl ring-1 ring-gray-100 shadow-sm px-4 py-3 flex items-center justify-between gap-3 hover:ring-emerald-200 transition-all">
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-gray-700 truncate">{r.tenant?.nome ?? '—'}</p>
                          <p className="text-[11px] text-gray-400 mt-0.5 tabular-nums">
                            {format(new Date(r.iniciadoEm), "dd/MM 'às' HH:mm", { locale: ptBR })}
                            {' · '}{r.ambientes.length} amb.
                            {r.finalizadoEm && <> · {Math.round((new Date(r.finalizadoEm).getTime() - new Date(r.iniciadoEm).getTime()) / 60000)} min</>}
                          </p>
                          {r.criadoPor && <p className="text-[11px] text-gray-400 truncate">{r.criadoPor.nome}</p>}
                        </div>
