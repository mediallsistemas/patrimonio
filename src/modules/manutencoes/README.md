...
| `finalizar` | `finalizar(tenantId: string, criadoPorId: string, id: string, input: FinalizarManutencaoInput)` | Só encontra registro `em_andamento` do **próprio criador** (`where: { id, tenantId, criadoPorId, status: 'em_andamento' }`); grava `fotoDepois`/`observacaoFinal`/`finalizadaEm`; `null` se não achou |
...
| `listarEmAndamentoDoUsuario` | `listarEmAndamentoDoUsuario(tenantId: string, criadoPorId: string)` | Pendentes de finalização do próprio usuário (retomada do fluxo) |
...
- Só o criador finaliza a própria manutenção (nem `tenant_admin` finaliza a de outro).
